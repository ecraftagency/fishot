package proto.message;

@javax.annotation.Generated("/Users/vunguyen/project/gdxsocket/core/src/proto/msg_profile.proto")
public final class PetLevelUpResponse
        implements io.protostuff.Message<PetLevelUpResponse> {

    private static final PetLevelUpResponse DEFAULT_INSTANCE = newBuilder().build();

    private int result;

    private int gold;

    private proto.message.PetRampageSkillResponse petRampageSkillResponse;

    private proto.message.PetRampageSkillResponse nextPetRampageSkillResponse;

    private int acc;

    private int accMax;

    private boolean unlock;

    private int unlockMoney;

    private int level;

    private int nextLevel;

    private int bonusGold;

    private boolean __merge_lock = false;
    private int __bitField0;

    private PetLevelUpResponse() {
        this.result = proto.message.PetResult.PET_RESULT_SUCCESS.getNumber();
        this.gold = 0;
        this.petRampageSkillResponse = proto.message.PetRampageSkillResponse.getDefaultInstance();
        this.nextPetRampageSkillResponse = proto.message.PetRampageSkillResponse.getDefaultInstance();
        this.acc = 0;
        this.accMax = 0;
        this.unlock = false;
        this.unlockMoney = 0;
        this.level = 0;
        this.nextLevel = 0;
        this.bonusGold = 0;
    }

    private PetLevelUpResponse(Builder builder) {
        __merge_lock = true;
    }

    public static Builder newBuilder() {
        return new Builder();
    }

    public static PetLevelUpResponse getDefaultInstance() {
        return DEFAULT_INSTANCE;
    }

    public static io.protostuff.Schema<PetLevelUpResponse> getSchema()
    {
    	return Schema.INSTANCE;
    }

    public proto.message.PetResult getResult() {
        return proto.message.PetResult.valueOf(result);
    }

    public int getResultValue() {
        return result;
    }

    public boolean hasResult() {
        return (__bitField0 & 1) == 1;
    }

    public PetLevelUpResponse withResult(proto.message.PetResult value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setResult(value)
            .build();
    }

    public int getGold() {
        return gold;
    }

    public boolean hasGold() {
        return (__bitField0 & 2) == 2;
    }

    public PetLevelUpResponse withGold(int value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setGold(value)
            .build();
    }

    public proto.message.PetRampageSkillResponse getPetRampageSkillResponse() {
        return petRampageSkillResponse;
    }

    public boolean hasPetRampageSkillResponse() {
        return (__bitField0 & 4) == 4;
    }

    public PetLevelUpResponse withPetRampageSkillResponse(proto.message.PetRampageSkillResponse value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setPetRampageSkillResponse(value)
            .build();
    }

    public proto.message.PetRampageSkillResponse getNextPetRampageSkillResponse() {
        return nextPetRampageSkillResponse;
    }

    public boolean hasNextPetRampageSkillResponse() {
        return (__bitField0 & 8) == 8;
    }

    public PetLevelUpResponse withNextPetRampageSkillResponse(proto.message.PetRampageSkillResponse value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setNextPetRampageSkillResponse(value)
            .build();
    }

    public int getAcc() {
        return acc;
    }

    public boolean hasAcc() {
        return (__bitField0 & 16) == 16;
    }

    public PetLevelUpResponse withAcc(int value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setAcc(value)
            .build();
    }

    public int getAccMax() {
        return accMax;
    }

    public boolean hasAccMax() {
        return (__bitField0 & 32) == 32;
    }

    public PetLevelUpResponse withAccMax(int value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setAccMax(value)
            .build();
    }

    public boolean getUnlock() {
        return unlock;
    }

    public boolean hasUnlock() {
        return (__bitField0 & 64) == 64;
    }

    public PetLevelUpResponse withUnlock(boolean value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setUnlock(value)
            .build();
    }

    public int getUnlockMoney() {
        return unlockMoney;
    }

    public boolean hasUnlockMoney() {
        return (__bitField0 & 128) == 128;
    }

    public PetLevelUpResponse withUnlockMoney(int value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setUnlockMoney(value)
            .build();
    }

    public int getLevel() {
        return level;
    }

    public boolean hasLevel() {
        return (__bitField0 & 256) == 256;
    }

    public PetLevelUpResponse withLevel(int value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setLevel(value)
            .build();
    }

    public int getNextLevel() {
        return nextLevel;
    }

    public boolean hasNextLevel() {
        return (__bitField0 & 512) == 512;
    }

    public PetLevelUpResponse withNextLevel(int value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setNextLevel(value)
            .build();
    }

    public int getBonusGold() {
        return bonusGold;
    }

    public boolean hasBonusGold() {
        return (__bitField0 & 1024) == 1024;
    }

    public PetLevelUpResponse withBonusGold(int value) {
        return PetLevelUpResponse.newBuilder()
            .mergeFrom(this)
            .setBonusGold(value)
            .build();
    }



    @Override
    public io.protostuff.Schema<PetLevelUpResponse> cachedSchema() {
        return Schema.INSTANCE;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (obj == null || this.getClass() != obj.getClass()) {
            return false;
        }
        PetLevelUpResponse that = (PetLevelUpResponse) obj;
        if (!java.util.Objects.equals(this.result, that.result)) {
            return false;
        }
        if (!java.util.Objects.equals(this.gold, that.gold)) {
            return false;
        }
        if (!java.util.Objects.equals(this.petRampageSkillResponse, that.petRampageSkillResponse)) {
            return false;
        }
        if (!java.util.Objects.equals(this.nextPetRampageSkillResponse, that.nextPetRampageSkillResponse)) {
            return false;
        }
        if (!java.util.Objects.equals(this.acc, that.acc)) {
            return false;
        }
        if (!java.util.Objects.equals(this.accMax, that.accMax)) {
            return false;
        }
        if (!java.util.Objects.equals(this.unlock, that.unlock)) {
            return false;
        }
        if (!java.util.Objects.equals(this.unlockMoney, that.unlockMoney)) {
            return false;
        }
        if (!java.util.Objects.equals(this.level, that.level)) {
            return false;
        }
        if (!java.util.Objects.equals(this.nextLevel, that.nextLevel)) {
            return false;
        }
        if (!java.util.Objects.equals(this.bonusGold, that.bonusGold)) {
            return false;
        }

        return true;
    }

    @Override
    public int hashCode() {
        int result = 1;
        result = 31 * result + Integer.hashCode(this.result);
        result = 31 * result + Integer.hashCode(this.gold);
        result = 31 * result + (this.petRampageSkillResponse == null ? 0 : this.petRampageSkillResponse.hashCode());
        result = 31 * result + (this.nextPetRampageSkillResponse == null ? 0 : this.nextPetRampageSkillResponse.hashCode());
        result = 31 * result + Integer.hashCode(this.acc);
        result = 31 * result + Integer.hashCode(this.accMax);
        result = 31 * result + Boolean.hashCode(this.unlock);
        result = 31 * result + Integer.hashCode(this.unlockMoney);
        result = 31 * result + Integer.hashCode(this.level);
        result = 31 * result + Integer.hashCode(this.nextLevel);
        result = 31 * result + Integer.hashCode(this.bonusGold);

        return result;
    }

    @Override
    public String toString() {
        java.util.List<String> parts = new java.util.ArrayList<>();
        if (hasResult()) {
            parts.add("result=" + getResult() + '(' + getResultValue() + ')');
        }
        if (hasGold()) {
            parts.add("gold=" + getGold());
        }
        if (hasPetRampageSkillResponse()) {
            parts.add("petRampageSkillResponse=" + getPetRampageSkillResponse());
        }
        if (hasNextPetRampageSkillResponse()) {
            parts.add("nextPetRampageSkillResponse=" + getNextPetRampageSkillResponse());
        }
        if (hasAcc()) {
            parts.add("acc=" + getAcc());
        }
        if (hasAccMax()) {
            parts.add("accMax=" + getAccMax());
        }
        if (hasUnlock()) {
            parts.add("unlock=" + getUnlock());
        }
        if (hasUnlockMoney()) {
            parts.add("unlockMoney=" + getUnlockMoney());
        }
        if (hasLevel()) {
            parts.add("level=" + getLevel());
        }
        if (hasNextLevel()) {
            parts.add("nextLevel=" + getNextLevel());
        }
        if (hasBonusGold()) {
            parts.add("bonusGold=" + getBonusGold());
        }
        return "PetLevelUpResponse{" + String.join(", ", parts) + "}";
    }

    public static final class Schema implements io.protostuff.Schema<PetLevelUpResponse>{

        private static final Schema INSTANCE = new Schema();

        private static final java.util.Map<String,Integer> __fieldMap = new java.util.HashMap<>();

        static {
        	__fieldMap.put("result", 1);
        	__fieldMap.put("gold", 2);
        	__fieldMap.put("petRampageSkillResponse", 3);
        	__fieldMap.put("nextPetRampageSkillResponse", 4);
        	__fieldMap.put("acc", 5);
        	__fieldMap.put("accMax", 6);
        	__fieldMap.put("unlock", 7);
        	__fieldMap.put("unlockMoney", 8);
        	__fieldMap.put("level", 9);
        	__fieldMap.put("nextLevel", 10);
        	__fieldMap.put("bonusGold", 11);
        }

        @Override
        public PetLevelUpResponse newMessage() {
            return new PetLevelUpResponse();
        }

        @Override
        public Class<PetLevelUpResponse> typeClass() {
            return PetLevelUpResponse.class;
        }

        @Override
        public String messageName() {
            return PetLevelUpResponse.class.getSimpleName();
        }

        @Override
        public String messageFullName() {
            return PetLevelUpResponse.class.getName();
        }

        @Override
        @Deprecated
        public boolean isInitialized(PetLevelUpResponse message) {
            return true;
        }

        @Override
        public void mergeFrom(io.protostuff.Input input, PetLevelUpResponse instance) throws java.io.IOException {
        	if (instance.__merge_lock) {
        		throw new IllegalStateException("Cannot reuse message instance");
        	} else {
        		instance.__merge_lock = true;
        	}
        	while(true) {
        		int number = input.readFieldNumber(this);
                if (number == 0) {
                    break;
                }
                switch(number) {
                    case 1:
                    	instance.result = input.readEnum();
                    	instance.__bitField0 |= 1;
                    	break;
                    case 2:
                    	instance.gold = input.readInt32();
                    	instance.__bitField0 |= 2;
                    	break;
                    case 3:
                    	instance.petRampageSkillResponse = input.mergeObject(null, proto.message.PetRampageSkillResponse.getSchema());
                    	instance.__bitField0 |= 4;
                    	break;
                    case 4:
                    	instance.nextPetRampageSkillResponse = input.mergeObject(null, proto.message.PetRampageSkillResponse.getSchema());
                    	instance.__bitField0 |= 8;
                    	break;
                    case 5:
                    	instance.acc = input.readInt32();
                    	instance.__bitField0 |= 16;
                    	break;
                    case 6:
                    	instance.accMax = input.readInt32();
                    	instance.__bitField0 |= 32;
                    	break;
                    case 7:
                    	instance.unlock = input.readBool();
                    	instance.__bitField0 |= 64;
                    	break;
                    case 8:
                    	instance.unlockMoney = input.readInt32();
                    	instance.__bitField0 |= 128;
                    	break;
                    case 9:
                    	instance.level = input.readInt32();
                    	instance.__bitField0 |= 256;
                    	break;
                    case 10:
                    	instance.nextLevel = input.readInt32();
                    	instance.__bitField0 |= 512;
                    	break;
                    case 11:
                    	instance.bonusGold = input.readInt32();
                    	instance.__bitField0 |= 1024;
                    	break;
                    default:
                        input.handleUnknownField(number, this);
                }
        	}
        }

        @Override
        public void writeTo(io.protostuff.Output output, PetLevelUpResponse instance) throws java.io.IOException {
            if((instance.__bitField0 & 1) == 1) {
            	output.writeEnum(1, instance.result, false);
            }

            if((instance.__bitField0 & 2) == 2) {
                output.writeInt32(2, instance.gold, false);
            }

            if((instance.__bitField0 & 4) == 4) {
            	output.writeObject(3, instance.petRampageSkillResponse, proto.message.PetRampageSkillResponse.getSchema(), false);
            }

            if((instance.__bitField0 & 8) == 8) {
            	output.writeObject(4, instance.nextPetRampageSkillResponse, proto.message.PetRampageSkillResponse.getSchema(), false);
            }

            if((instance.__bitField0 & 16) == 16) {
                output.writeInt32(5, instance.acc, false);
            }

            if((instance.__bitField0 & 32) == 32) {
                output.writeInt32(6, instance.accMax, false);
            }

            if((instance.__bitField0 & 64) == 64) {
                output.writeBool(7, instance.unlock, false);
            }

            if((instance.__bitField0 & 128) == 128) {
                output.writeInt32(8, instance.unlockMoney, false);
            }

            if((instance.__bitField0 & 256) == 256) {
                output.writeInt32(9, instance.level, false);
            }

            if((instance.__bitField0 & 512) == 512) {
                output.writeInt32(10, instance.nextLevel, false);
            }

            if((instance.__bitField0 & 1024) == 1024) {
                output.writeInt32(11, instance.bonusGold, false);
            }

        }

        @Override
        public String getFieldName(int number) {
        	switch(number) {
        		case 1: return "result";
        		case 2: return "gold";
        		case 3: return "petRampageSkillResponse";
        		case 4: return "nextPetRampageSkillResponse";
        		case 5: return "acc";
        		case 6: return "accMax";
        		case 7: return "unlock";
        		case 8: return "unlockMoney";
        		case 9: return "level";
        		case 10: return "nextLevel";
        		case 11: return "bonusGold";
        		default: return null;
        	}
        }

        @Override
        public int getFieldNumber(String name) {
        	final Integer number = __fieldMap.get(name);
        	return number == null ? 0 : number.intValue();
        }

    }

    public static final class Builder  {

        private int result;

        private int gold;

        private proto.message.PetRampageSkillResponse petRampageSkillResponse;

        private proto.message.PetRampageSkillResponse nextPetRampageSkillResponse;

        private int acc;

        private int accMax;

        private boolean unlock;

        private int unlockMoney;

        private int level;

        private int nextLevel;

        private int bonusGold;

        private int __bitField0;

        private Builder() {
            this.result = proto.message.PetResult.PET_RESULT_SUCCESS.getNumber();
            this.gold = 0;
            this.petRampageSkillResponse = proto.message.PetRampageSkillResponse.getDefaultInstance();
            this.nextPetRampageSkillResponse = proto.message.PetRampageSkillResponse.getDefaultInstance();
            this.acc = 0;
            this.accMax = 0;
            this.unlock = false;
            this.unlockMoney = 0;
            this.level = 0;
            this.nextLevel = 0;
            this.bonusGold = 0;
        }

        public Builder mergeFrom(PetLevelUpResponse instance) {
            if (instance.hasResult()) {
                this.setResult(instance.getResult());
            }

            if (instance.hasGold()) {
                this.setGold(instance.getGold());
            }

            if (instance.hasPetRampageSkillResponse()) {
                this.setPetRampageSkillResponse(instance.getPetRampageSkillResponse());
            }

            if (instance.hasNextPetRampageSkillResponse()) {
                this.setNextPetRampageSkillResponse(instance.getNextPetRampageSkillResponse());
            }

            if (instance.hasAcc()) {
                this.setAcc(instance.getAcc());
            }

            if (instance.hasAccMax()) {
                this.setAccMax(instance.getAccMax());
            }

            if (instance.hasUnlock()) {
                this.setUnlock(instance.getUnlock());
            }

            if (instance.hasUnlockMoney()) {
                this.setUnlockMoney(instance.getUnlockMoney());
            }

            if (instance.hasLevel()) {
                this.setLevel(instance.getLevel());
            }

            if (instance.hasNextLevel()) {
                this.setNextLevel(instance.getNextLevel());
            }

            if (instance.hasBonusGold()) {
                this.setBonusGold(instance.getBonusGold());
            }

            return this;
        }

        public proto.message.PetResult getResult() {
            return proto.message.PetResult.valueOf(result);
        }

        public int getResultValue() {
            return result;
        }

        public Builder setResult(proto.message.PetResult value) {
            if (value == null) {
                throw new NullPointerException("Cannot set PetLevelUpResponse#result to null");
            }

            if (value == proto.message.PetResult.UNRECOGNIZED) {
                throw new IllegalArgumentException("Cannot set PetLevelUpResponse#result to UNRECOGNIZED");
            }

            this.result = value.getNumber();
            __bitField0 |= 1;
            return this;
        }

        public Builder setResultValue(int value) {
            this.result = value;
            __bitField0 |= 1;
            return this;
        }

        public Builder clearResult() {
            this.result = 0;
            __bitField0 &= ~1;
            return this;
        }

        public boolean hasResult() {
            return (__bitField0 & 1) == 1;
        }

        public int getGold() {
            return gold;
        }

        public Builder setGold(int value) {
            this.gold = value;
            __bitField0 |= 2;
            return this;
        }

        public Builder clearGold() {
            this.gold = 0;
            __bitField0 &= ~2;
            return this;
        }

        public boolean hasGold() {
            return (__bitField0 & 2) == 2;
        }

        public proto.message.PetRampageSkillResponse getPetRampageSkillResponse() {
            return petRampageSkillResponse;
        }

        public Builder setPetRampageSkillResponse(proto.message.PetRampageSkillResponse value) {
            if (value == null) {
                throw new NullPointerException("Cannot set PetLevelUpResponse#petRampageSkillResponse to null");
            }

            this.petRampageSkillResponse = value;
            __bitField0 |= 4;
            return this;
        }

        public Builder clearPetRampageSkillResponse() {
            this.petRampageSkillResponse = proto.message.PetRampageSkillResponse.getDefaultInstance();
            __bitField0 &= ~4;
            return this;
        }

        public boolean hasPetRampageSkillResponse() {
            return (__bitField0 & 4) == 4;
        }

        public proto.message.PetRampageSkillResponse getNextPetRampageSkillResponse() {
            return nextPetRampageSkillResponse;
        }

        public Builder setNextPetRampageSkillResponse(proto.message.PetRampageSkillResponse value) {
            if (value == null) {
                throw new NullPointerException("Cannot set PetLevelUpResponse#nextPetRampageSkillResponse to null");
            }

            this.nextPetRampageSkillResponse = value;
            __bitField0 |= 8;
            return this;
        }

        public Builder clearNextPetRampageSkillResponse() {
            this.nextPetRampageSkillResponse = proto.message.PetRampageSkillResponse.getDefaultInstance();
            __bitField0 &= ~8;
            return this;
        }

        public boolean hasNextPetRampageSkillResponse() {
            return (__bitField0 & 8) == 8;
        }

        public int getAcc() {
            return acc;
        }

        public Builder setAcc(int value) {
            this.acc = value;
            __bitField0 |= 16;
            return this;
        }

        public Builder clearAcc() {
            this.acc = 0;
            __bitField0 &= ~16;
            return this;
        }

        public boolean hasAcc() {
            return (__bitField0 & 16) == 16;
        }

        public int getAccMax() {
            return accMax;
        }

        public Builder setAccMax(int value) {
            this.accMax = value;
            __bitField0 |= 32;
            return this;
        }

        public Builder clearAccMax() {
            this.accMax = 0;
            __bitField0 &= ~32;
            return this;
        }

        public boolean hasAccMax() {
            return (__bitField0 & 32) == 32;
        }

        public boolean getUnlock() {
            return unlock;
        }

        public Builder setUnlock(boolean value) {
            this.unlock = value;
            __bitField0 |= 64;
            return this;
        }

        public Builder clearUnlock() {
            this.unlock = false;
            __bitField0 &= ~64;
            return this;
        }

        public boolean hasUnlock() {
            return (__bitField0 & 64) == 64;
        }

        public int getUnlockMoney() {
            return unlockMoney;
        }

        public Builder setUnlockMoney(int value) {
            this.unlockMoney = value;
            __bitField0 |= 128;
            return this;
        }

        public Builder clearUnlockMoney() {
            this.unlockMoney = 0;
            __bitField0 &= ~128;
            return this;
        }

        public boolean hasUnlockMoney() {
            return (__bitField0 & 128) == 128;
        }

        public int getLevel() {
            return level;
        }

        public Builder setLevel(int value) {
            this.level = value;
            __bitField0 |= 256;
            return this;
        }

        public Builder clearLevel() {
            this.level = 0;
            __bitField0 &= ~256;
            return this;
        }

        public boolean hasLevel() {
            return (__bitField0 & 256) == 256;
        }

        public int getNextLevel() {
            return nextLevel;
        }

        public Builder setNextLevel(int value) {
            this.nextLevel = value;
            __bitField0 |= 512;
            return this;
        }

        public Builder clearNextLevel() {
            this.nextLevel = 0;
            __bitField0 &= ~512;
            return this;
        }

        public boolean hasNextLevel() {
            return (__bitField0 & 512) == 512;
        }

        public int getBonusGold() {
            return bonusGold;
        }

        public Builder setBonusGold(int value) {
            this.bonusGold = value;
            __bitField0 |= 1024;
            return this;
        }

        public Builder clearBonusGold() {
            this.bonusGold = 0;
            __bitField0 &= ~1024;
            return this;
        }

        public boolean hasBonusGold() {
            return (__bitField0 & 1024) == 1024;
        }




        @Override
        public boolean equals(Object obj) {
            if (this == obj) {
                return true;
            }
            if (obj == null || this.getClass() != obj.getClass()) {
                return false;
            }
            Builder that = (Builder) obj;
            if (!java.util.Objects.equals(this.result, that.result)) {
                return false;
            }
            if (!java.util.Objects.equals(this.gold, that.gold)) {
                return false;
            }
            if (!java.util.Objects.equals(this.petRampageSkillResponse, that.petRampageSkillResponse)) {
                return false;
            }
            if (!java.util.Objects.equals(this.nextPetRampageSkillResponse, that.nextPetRampageSkillResponse)) {
                return false;
            }
            if (!java.util.Objects.equals(this.acc, that.acc)) {
                return false;
            }
            if (!java.util.Objects.equals(this.accMax, that.accMax)) {
                return false;
            }
            if (!java.util.Objects.equals(this.unlock, that.unlock)) {
                return false;
            }
            if (!java.util.Objects.equals(this.unlockMoney, that.unlockMoney)) {
                return false;
            }
            if (!java.util.Objects.equals(this.level, that.level)) {
                return false;
            }
            if (!java.util.Objects.equals(this.nextLevel, that.nextLevel)) {
                return false;
            }
            if (!java.util.Objects.equals(this.bonusGold, that.bonusGold)) {
                return false;
            }

            return true;
        }

        @Override
        public int hashCode() {
            int result = 1;
            result = 31 * result + Integer.hashCode(this.result);
            result = 31 * result + Integer.hashCode(this.gold);
            result = 31 * result + (this.petRampageSkillResponse == null ? 0 : this.petRampageSkillResponse.hashCode());
            result = 31 * result + (this.nextPetRampageSkillResponse == null ? 0 : this.nextPetRampageSkillResponse.hashCode());
            result = 31 * result + Integer.hashCode(this.acc);
            result = 31 * result + Integer.hashCode(this.accMax);
            result = 31 * result + Boolean.hashCode(this.unlock);
            result = 31 * result + Integer.hashCode(this.unlockMoney);
            result = 31 * result + Integer.hashCode(this.level);
            result = 31 * result + Integer.hashCode(this.nextLevel);
            result = 31 * result + Integer.hashCode(this.bonusGold);

            return result;
        }

        @Override
        public String toString() {
            java.util.List<String> parts = new java.util.ArrayList<>();
            if (hasResult()) {
                parts.add("result=" + getResult() + '(' + getResultValue() + ')');
            }
            if (hasGold()) {
                parts.add("gold=" + getGold());
            }
            if (hasPetRampageSkillResponse()) {
                parts.add("petRampageSkillResponse=" + getPetRampageSkillResponse());
            }
            if (hasNextPetRampageSkillResponse()) {
                parts.add("nextPetRampageSkillResponse=" + getNextPetRampageSkillResponse());
            }
            if (hasAcc()) {
                parts.add("acc=" + getAcc());
            }
            if (hasAccMax()) {
                parts.add("accMax=" + getAccMax());
            }
            if (hasUnlock()) {
                parts.add("unlock=" + getUnlock());
            }
            if (hasUnlockMoney()) {
                parts.add("unlockMoney=" + getUnlockMoney());
            }
            if (hasLevel()) {
                parts.add("level=" + getLevel());
            }
            if (hasNextLevel()) {
                parts.add("nextLevel=" + getNextLevel());
            }
            if (hasBonusGold()) {
                parts.add("bonusGold=" + getBonusGold());
            }
            return "PetLevelUpResponse{" + String.join(", ", parts) + "}";
        }

        public PetLevelUpResponse build() {
            proto.message.PetLevelUpResponse result = new proto.message.PetLevelUpResponse();
            result.__bitField0 = __bitField0;
            result.result = this.result;
            result.gold = this.gold;
            result.petRampageSkillResponse = this.petRampageSkillResponse;
            result.nextPetRampageSkillResponse = this.nextPetRampageSkillResponse;
            result.acc = this.acc;
            result.accMax = this.accMax;
            result.unlock = this.unlock;
            result.unlockMoney = this.unlockMoney;
            result.level = this.level;
            result.nextLevel = this.nextLevel;
            result.bonusGold = this.bonusGold;
            return result;
        }

    }



}